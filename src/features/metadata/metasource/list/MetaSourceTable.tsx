...
import { META_SOURCE_LIST_PREFIX } from '../metasource.cache';
import { createMetaSourceColumns } from './metasource.columns';
import { MetaSourceTableToolbar } from './MetaSourceTableToolbar';

export type MetaSourceTableProps = {
  tableMode?: TableModeOptions;
};

const getRowId = (row: MetaSource) => row.id;

export function MetaSourceTable({ tableMode }: MetaSourceTableProps) {
  const navigate = useNavigate();
  const location = useLocation();
  const [pendingDelete, setPendingDelete] = useState<MetaSource | null>(null);
  const [bulkDeleteOpen, setBulkDeleteOpen] = useState(false);

  const dc = useDataCollection<MetaSource>({
    queryKey: [...META_SOURCE_LIST_PREFIX],
    queryFn: metaSourceApi.search,
    getRowId,
    tableMode,
  });
  ...
  const goToEdit = (
    ...
    goto(`/meta-sources/${rowId}/edit${suffix}`);
  };

  const deleteMutation = useMutation({
    mutationFn: (id: string) => metaSourceApi.delete(id),
    onSuccess: (_void, id) => {
      dc.removeItem(id);
  ...
  const handleDelete = (item: MetaSource) => setPendingDelete(item);
  ...
  const columns = useMemo(
    () =>
      createMetaSourceColumns({
        onView: (item) => goto(`/meta-sources/${item.id}`),
        onViewFullscreen: (item) => goto(`/meta-sources/${item.id}/dialog`),
  ...
  const table = useAppTable<MetaSource>({ dc, columns, getRowId });
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
      <MetaSourceTableToolbar
        keyword={dc.globalFilter}
        selectedCount={dc.selectedIds.length}
        onKeywordChange={(value) => dc.setGlobalFilter(value)}
        onCreateDialog={() => goto('/meta-sources/new/dialog')}
        onCreateFullscreen={() => goto('/meta-sources/new')}
      ...
      <ConfirmDialog
        ...
        description={
          <>
            ...
            MetaSource đã chọn? Hành động này không hoàn tác được.
          </>
        }
