...
const getRowId = (row: Organization) => row.id;

export function OrganizationTable({ tableMode }: OrganizationTableProps) {
  ...
  const goToEdit = (
    ...
    goto(`/organizations/${rowId}/edit${suffix}`);
  };
  ...
  const handleDelete = (item: Organization) => setPendingDelete(item);
  ...
  const columns = useMemo(
    () =>
      createOrganizationColumns({
        onView: (item) => goto(`/organizations/${item.id}`),
        onViewFullscreen: (item) => goto(`/organizations/${item.id}/dialog`),
