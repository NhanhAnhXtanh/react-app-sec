...
/**
 * Spring Data's Page<T> shape — what /api/* list endpoints actually return.
 * We translate this into the template's PageResponse<T> at the api boundary
 * so feature code stays unchanged.
 */
...
/**
 * Build query string for Spring Pageable: `page`, `size`, `sort=field,dir`.
 * Also appends `keyword` and `filters` so backend custom search can receive them.
 */
