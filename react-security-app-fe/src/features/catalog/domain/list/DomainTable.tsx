...
import { DOMAIN_LIST_PREFIX } from '../domain.cache';
...
const getRowId = (row: Domain) => row.id;

export function DomainTable({ tableMode }: DomainTableProps) {
  const navigate = useNavigate();
  const location = useLocation();
  const [pendingDelete, setPendingDelete] = useState<Domain | null>(null);
  const [bulkDeleteOpen, setBulkDeleteOpen] = useState(false);

  const dc = useDataCollection<Domain>({
    queryKey: [...DOMAIN_LIST_PREFIX],
    queryFn: domainApi.search,
    getRowId,
    tableMode,
  });
  ...
  const goToEdit = (
    ...
    goto(`/domains/${rowId}/edit${suffix}`);
  };
  ...
  const handleDelete = (item: Domain) => setPendingDelete(item);
  ...
  const columns = useMemo(
    () =>
      createDomainColumns({
        onView: (item) => goto(`/domains/${item.id}`),
        onViewFullscreen: (item) => goto(`/domains/${item.id}/dialog`),
  ...
  const table = useAppTable<Domain>({ dc, columns, getRowId });
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-card shadow-sm">
      <DomainTableToolbar
        ...
        onCreateDialog={() => goto('/domains/new/dialog')}
        onCreateFullscreen={() => goto('/domains/new')}
